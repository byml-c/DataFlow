...
class MessageHandler(Handler):
    ...
    def __init__(self, input_path, output_path='./output.txt'):
...
'''聊天记录片段：\n{messages}\n\n根据聊天记录内容，严格按照输出格式，我的输出如下：\n'''
    ...
    def run(self, block:list, idx:int, retry=5):
        ...
        while retry > 0:
            ...
            try:
                ...
                for r in res:
                    ...
                    results.append(format_r)
                self.finish(idx, results)
                return 
    ...
    def read(self):
        path = f'{self.output_name}-save.json'
        if not os.path.exists(path):
            return False
        self_obj = json.load(open(path, 'r', encoding='utf-8'))

        for key in ['qa', 'error', 'blocks', 'chains', 'messages', 'user_map']:
            if self_obj.get(key) is None:
                return False
        ...
        self.index = max(self.index, self_obj.get('index', -1))
        return True

    def initialize(self):
        '''从源文件中读取聊天记录并进行初步过滤和脱敏处理'''
        self.log.log('初始化处理聊天记录，路径：{}'.format(self.input_path))
        self.load()
    ...
    def load(self):
        ...
        message_list = open(self.input_path, 'r', encoding='utf-8').readlines()
...
if __name__ == '__main__':
    h = MessageHandler('../RunData\\QQ\\南哪2022级咨询交流②群.qa', '.\\temp\\BATCH01\\QQ\\南哪2022级咨询交流②群.qa')
    h.handle('qwen1.5-32b-chat')